...
angular.module('epos')
  .controller('StockCategoryListCtrl', ['$rootScope','$scope', '$state', 'uiGridConstants', '_', 'safeApply', 'openErp','product.category', 'toastMess',
	   ...
	   var loadAllCategory = function(){
			  ...
			  $openErp.doSearch($productCategory.model, null, [fieldsProductCates.id, fieldsProductCates.complete_name], {
			  	...
			  	$safeApply($scope, function(){
		        	...
		        	$rootScope.showLoading = false; // hidden loading
		        });
			  }, function(error){
			  	$safeApply($scope, function(){
			  		$rootScope.showLoading = false; // hidden loading
	   ...
	   onRegisterApi: function(gridApi){
	    ...
	    appScopeProvider: { 
	          onDblClick : function(row) {
               ...
               $rootScope.showLoading = true; // show loading
               // go to detail page
               $state.transitionTo('stock.category.detail', { id:  id});
	          }
	    },
	  ...
	  $scope.goToCreateCategory = function(){
	  	$safeApply($scope, function(){
	  		$state.transitionTo('stock.category.create');
	  	});
	  }
